...
return images.map((item)=>{
 return(
     <div>
          ...
          <div style={{position:'absolute',left:15,fontWeight:'bold'}}>{item.title}</div>
          <div style={{position:'absolute',left:0,top:25,fontWeight:600,fontSize:14}}>{item.description}</div>
          </div>
          <img src={`${ServerURL}/images/${item.image}`} style={{borderRadius:20,width:150,height:75,marginTop:60,marginLeft:45}} />
          </Paper>
     </div>
 )
})
...
<div style={{display:'flex',justifyContent:'space-between',paddingTop:15,paddingBottom:15,width:'98%'}}>
    <span style={{fontWeight:'bold',fontSize:24,}}>Why Pnp</span>
